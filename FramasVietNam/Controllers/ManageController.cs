...
namespace FramasVietNam.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        public async Task<ActionResult> VerifyPhoneNumber(VerifyPhoneNumberViewModel model)
        {
            ...
            ModelState.AddModelError("", Language.label("FailedToVerifyPhone"));
            return View(model);
        }
        ...
        public ActionResult ChangeEmail()
        {
            ...
            PassingDataToChangeEmail();
            return View();
        }
        private void PassingDataToChangeEmail()
        {
            //check email User
            string strUser = User.Identity.GetUserName();
            AspNetUser objUser = db_user.AspNetUsers.SingleOrDefault(m => m.UserName == strUser);
        ...
        [HttpPost]
        public ActionResult ChangeEmail(FormCollection fc)
        {
            string txtEmail = mFunction.ToString(fc["txtEmail"]);
            if (txtEmail.Length <= 0)
            {
                ViewBag.Message = "► " + Language.label("PleaseInputData") + ": Email";
                //Passing data to View
                PassingDataToChangeEmail();
                return View();
            }
        ...
        //
        // GET: /Manage/ChangePassword
        public ActionResult ManagerLanguage()
        {
            return View();
        }
        ...
        public async Task<ActionResult> ChangePassword(ChangePasswordViewModel model)
        {
            ...
            if (!ModelState.IsValid)
            {
                ...
                ViewBag.Message = "► " + Language.label("InvalidCode");
                return View(model);
            }
            var result = await UserManager.ChangePasswordAsync(User.Identity.GetUserName(), model.OldPassword, model.NewPassword);
            if (result.Succeeded)
            {
                var user = await UserManager.FindByIdAsync(User.Identity.GetUserId());
                if (user != null)
