...
{
    public class SendNotificationsLoginM1
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static List<tbl_LogFinger_Login_M1> GetNotificationLoginM1()
        {
            try
            {
                ...
                using (var connection = new SqlConnection(connString))
                {
                    ...
                    using (command = new SqlCommand(@"SELECT ID,MachineNumber,IndRegID,VerifyMode,InOutMode,DateTimeRecord FROM [dbo].[tbl_LogFinger_Login_M1] order by ID desc", connection))
                    {
                        ...
                        if (dependency == null)
                        {
                            ...
                            dependency.OnChange += new OnChangeEventHandler(Dependency_OnChangeLoginM1);
                        }
        ...
        private static void Dependency_OnChangeLoginM1(object sender, SqlNotificationEventArgs e)
        {
            if (dependency != null)
            {
                dependency.OnChange -= Dependency_OnChangeLoginM1;
                dependency = null;
            }
